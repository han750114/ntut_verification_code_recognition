...
def get_next_batch(img_files, img_labels, batch_size):
    batch_x = np.zeros([batch_size, IMAGE_WIDTH * IMAGE_HEIGHT * 3])  # 乘以3是因為您的圖像是RGB，每個像素有三個通道
    ...
    for i in range(batch_size):
        ...
        image = cv2.resize(image, (IMAGE_WIDTH, IMAGE_HEIGHT))  # 調整圖片大小
...
X = tf.keras.Input(shape=(IMAGE_HEIGHT, IMAGE_WIDTH, 1), dtype=tf.float32)
Y = tf.keras.Input(shape=(CAPTCHA_LEN * CHAR_SET_LEN,), dtype=tf.float32)
keep_prob_scalar = 0.5  # 或者使用其他你選擇的保留率
...
def crack_captcha_cnn_network(w_alpha=0.01, b_alpha=0.1):
    x = tf.keras.layers.Input(shape=(IMAGE_HEIGHT, IMAGE_WIDTH, 1), dtype=tf.float32)
...
model = tf.keras.models.Model(inputs=X, outputs=model)

# 損失函式、優化器
logits = model(X, training=True)  # 使用模型輸出來計算損失
...
for step in range(step_cnt):
    ...
    if step % 100 == 0:
        ...
        logits_test = model(batch_x_test, training=False)  # 推理模式下
        ...
        # 儲存模型
        model.save_weights('/Users/shuha/OneDrive/Desktop/tensorflow final project/crack_captcha', save_format='tf', overwrite=True)
